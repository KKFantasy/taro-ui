...
export function handleSubscripts(args, item) {
  const { options } = args;
  const { _value } = item;
  ...
  for (const key in subscripts) {
    if (subscripts.hasOwnProperty(key)) {
      ...
      let newKey = dayjs(key).startOf('day').format('YYYY-MM-DD');
      obj[newKey] = v;
    }
  }
  item.subscripts = obj;
...
export default [handleActive, handleMarks, handleDisabled, handleValid, handleSubscripts];